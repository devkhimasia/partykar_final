	...
	$.fn.Selectyze2 = function(opt) {
		...
		return this.each(function() {
			...
			var 
				...
				DivSelect = $('<div>', {'class' : 'DivSelectyze2 '+opts.theme+''}),
				UlSelect = $('<ul>',{'class':'UlSelectize2'}),
				liHtml = '';
			...
			DivSelect.html('<a href="#" rel="'+optionselected.val()+'" class="selectyzeValue2">'+optionselected.text()+'</a>');

			UlSelect.appendTo(DivSelect).html(liHtml);
			$('.DivSelectyze2').each(function(i,el){
			...
			DivSelect.find('a.selectyzeValue2').click(function(e){
				e.preventDefault();
				closeList($('ul.UlSelectize2').not($(this).next()));
				openList($(this).next('ul.UlSelectize2'));
			});
			
			UlSelect.find('a').click(function(e){
				e.preventDefault();
				DivSelect.find('a.selectyzeValue2').text($(this).text());
				$this.val($(this).attr('rel'));           
				$this.trigger('change');         
				closeList($this.next().find('.UlSelectize2'));
			});
			
			$(document).click(function(e){if(n) closeList($('.UlSelectize2').not(':hidden'));});
