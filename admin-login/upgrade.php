...
function update(id)
{
	...
	var queryString = "?id="+rid;
	ajaxRequest.open("GET","activate.php" + queryString, true);
...
function filter(alpha)
{
	...
	{
	window.location="upgrade.php?alpha="+alpha;
	}
}
function search1()
{
	var name=document.getElementById('search').value;
	window.location="upgrade.php?search="+name;
...
<div class="mainH_strip gradientGray boxShadow">
    ...
    <a href="upgrade.php" target="_self" class="active">UPGRADES</a> 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_full" id="main_tab">
  <tr>
    ...
    <td class="w10 tdBlock mainHeading">paid/Unpaid&nbsp;<img src="images/down-arrow.png" /></td>
  
    <td class="w12 tdBlock mainHeading">Date/Time&nbsp;<img src="images/down-arrow.png" /></td>
    <td class="w6 tdBlock mainHeading">Activate&nbsp;<img src="images/down-arrow.png"/></td>
  </tr>
...
{
	$res=mysql_query("select v.company,c.category,c.area,c.membership_type,c.membership_period,c.price,c.paid,c.date_added,c.id,c.vendor_id from vendor v,vendor_category c where v.id=c.vendor_id and activate='no'",$db_con);
}
else
{
	...
	{
		$alpha=$_GET['alpha'];
		$res=mysql_query ("select v.company,c.category,c.area,c.membership_type,c.membership_period,c.price,c.paid,c.date_added,c.id,c.vendor_id from vendor v,vendor_category c where v.id=c.vendor_id and activate='no' and v.company like '$alpha%'",$db_con);
	}
	else
	{
		$search=$_GET['search'];
		$res=mysql_query ("select v.company,c.category,c.area,c.membership_type,c.membership_period,c.price,c.paid,c.date_added,c.id,c.vendor_id from vendor v,vendor_category c where v.id=c.vendor_id and activate='no' and v.company like '%$search%'",$db_con);
	}
  ...
  while($row=mysql_fetch_row($res))
  {
	  echo "
	   <tr>
    <td class='w14 tdBlock vinfotab'><a class='block1 fancybox.iframe'  href='popup_data.php?id=$row[9]'>$row[0]</a></td>
    ...
    <td class='w10 tdBlock vinfotab'>$row[6]</td>
    
    <td class='w12 tdBlock vinfotab'>$row[7]</td>
    <td class='w6 tdBlock vinfotab'><img src='images/deactive.png' onClick='update($row[8])'/></td>
  </tr>
	  ";
  }
  ?>
