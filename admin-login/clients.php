...
{
	$res=mysql_query ("select v.company,c.category,c.area,c.membership_type,c.membership_period,c.price,c.doa,c.doe,v.id from vendor v,vendor_category c where v.id=c.vendor_id and activate='yes' and c.membership_type!='free' order by company",$db_con);
}
else
{
	...
	{
		$alpha=$_GET['alpha'];
		$res=mysql_query ("select v.company,c.category,c.area,c.membership_type,c.membership_period,c.price,c.doa,c.doe,v.id from vendor v,vendor_category c where v.id=c.vendor_id and activate='yes' and c.membership_type!='free' and v.company like '$alpha%' order by company",$db_con);
	}
	else
	{
		$search=$_GET['search'];
		$res=mysql_query ("select v.company,c.category,c.area,c.membership_type,c.membership_period,c.price,c.doa,c.doe,v.id from vendor v,vendor_category c where v.id=c.vendor_id and activate='yes' and c.membership_type!='free' and v.company like '%$search%' order by company",$db_con);
	}
...
function filter(alpha)
{
	...
	{
	window.location="clients.php?alpha="+alpha;
	}
}
function search1()
{
	var name=document.getElementById('search').value;
	window.location="clients.php?search="+name;
...
<div class="mainH_strip gradientGray boxShadow">
    ...
    <a href="clients.php" target="_self" class="active">CLIENTS</a> 
